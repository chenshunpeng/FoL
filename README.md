...
For memory-limited evaluation, add `--efficient_ram_testing`. It preserves float32 features, exact chunked `IndexFlatL2` retrieval, and the original local reranking, while caching only query and retrieved-candidate local features on SSD. `--efficient_ram_max_cache_gib` and `--efficient_ram_min_free_gib` default to 500 GB, and reranking depth follows `--recall_values`.

For [SF-XL](https://github.com/gmberton/cosplace) at 322×322, regular mode would require about $\color{red}{\mathbf{4816.54\ GB}}$ RAM for database local features and $\color{red}{\mathbf{88.30\ GB}}$ for database global features. Low-memory top-100 reranking uses at most about $\color{red}{\mathbf{371.03\ GB}}$ temporary SSD cache ($\color{red}{\mathbf{3.67\ GB}}$ query local + $\color{red}{\mathbf{367.36\ GB}}$ candidate local) and only a few GB extra CPU RAM.
