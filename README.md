...
For memory-limited evaluation, add `--efficient_ram_testing`. It preserves float32 features, exact chunked `IndexFlatL2` retrieval, and the original local reranking, while caching only query and retrieved-candidate local features. Use `--efficient_ram_cache_dir` to select the temporary directory. `--efficient_ram_max_cache_gib` and `--efficient_ram_min_free_gib` both default to $\color{red}{\mathbf{500\ GiB}}$, and reranking depth follows `--recall_values`.
